...
class TestFlowManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_pending_function_calls_counter(self):
        """Test that pending function calls counter works correctly."""
        flow_manager = FlowManager(
            task=self.mock_task,
            llm=self.mock_llm,
            context_aggregator=self.mock_context_aggregator,
        ...
        # Create a node with function that will be called
        node_config: NodeConfig = {
            "task_messages": [{"role": "system", "content": "Test"}],
            "functions": [
                {
        ...
        self.assertEqual(flow_manager._pending_function_calls, 0)

        # When we call the function, we need to capture and execute the context_updated callback
        # to properly complete the function call flow
        context_updated_callback = None
        ...
        # Call function
        await func("test_function", "id", {}, None, None, result_callback)

        # Counter should have incremented
        self.assertEqual(flow_manager._pending_function_calls, 1)

        # Now execute the context_updated callback to complete the function call
        self.assertIsNotNone(context_updated_callback, "Context updated callback not set")
        await context_updated_callback()

        # Now counter should be decremented back to 0
        self.assertEqual(flow_manager._pending_function_calls, 0)

        # Test multiple simultaneous calls
        context_updated_callbacks = []

        async def multi_result_callback(result, properties=None):
            nonlocal context_updated_callbacks
            if properties and properties.on_context_updated:
                context_updated_callbacks.append(properties.on_context_updated)

        # Start multiple calls
        await func("test_function", "id1", {}, None, None, multi_result_callback)
        await func("test_function", "id2", {}, None, None, multi_result_callback)

        # Counter should reflect both pending calls
        self.assertEqual(flow_manager._pending_function_calls, 2)
        self.assertEqual(len(context_updated_callbacks), 2)

        # Complete first call
        await context_updated_callbacks[0]()
        self.assertEqual(flow_manager._pending_function_calls, 1)

        # Complete second call
        await context_updated_callbacks[1]()
        self.assertEqual(flow_manager._pending_function_calls, 0)
